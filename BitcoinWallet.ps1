...
class ECDSA {
    ...
    # BEGIN fixed-generator table methods
    hidden static [void] EnsureGeneratorTable() {
        ...
        try {
            if ( $null -ne [ECDSA]::GeneratorTable ) { return }
            $coordinates = [Secp256k1GeneratorData]::Coordinates -split '\s+'
            [ECDSAJ[]]$table = [ECDSAJ[]]::new( 256 )
            for ( [int]$i = 0; $i -lt 256; $i++ ) {
                # Prefix zero to prevent signed hexadecimal parsing of high-bit coordinates.
                $coordinateX = [bigint]::Parse( '0' + $coordinates[2 * $i], 'AllowHexSpecifier' )
                $coordinateY = [bigint]::Parse( '0' + $coordinates[2 * $i + 1], 'AllowHexSpecifier' )
                $table[$i] = [ECDSAJ]::new( $coordinateX, $coordinateY, [bigint]::One )
            }
            [ECDSA]::GeneratorTable = $table
    ...
    # END fixed-generator table methods
...
class Secp256k1GeneratorData {
    <#
    G, 2G, ..., 2^255G. Each line contains unsigned 64-digit hexadecimal X and Y.
    Regenerate all 256 lines with PowerShell 5.1 or later (no wallet functions required):

```powershell
$prime = [bigint]::Pow(2, 256) - [bigint]::Pow(2, 32) - 977
$x = [bigint]::Parse('079be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798', 'AllowHexSpecifier')
$y = [bigint]::Parse('0483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8', 'AllowHexSpecifier')
for ($i = 0; $i -lt 256; $i++) {
    $hexX = $x.ToString('x').TrimStart('0').PadLeft(64, '0')
    $hexY = $y.ToString('x').TrimStart('0').PadLeft(64, '0')
    "$hexX $hexY"
    $slope = (3 * $x * $x * [bigint]::ModPow(2 * $y, $prime - 2, $prime)) % $prime
    $nextX = (($slope * $slope - 2 * $x) % $prime + $prime) % $prime
    $y = (($slope * ($x - $nextX) - $y) % $prime + $prime) % $prime
    $x = $nextX
}
```
    #>
    hidden static [string] $Coordinates = @'
