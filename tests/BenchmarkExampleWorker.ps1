param([string]$SourceRoot,[string]$Example,[ValidateSet('generate','load','seed','embedded')][string]$Mode,[string]$CachePath,[string]$ResultPath)
...
if ($Mode -eq 'embedded' -and (Test-Path $CachePath)) { throw 'Embedded table must not create a cache file' }
[pscustomobject]@{Example=$Example;Mode=$Mode;ImportMilliseconds=$importTimer.Elapsed.TotalMilliseconds;PowerShell=$PSVersionTable.PSVersion.ToString();Measurements=$measurements} |
